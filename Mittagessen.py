...
def mittagessen_aufruf(message):
    ...
    if kommtMit == "Ja":
	message.send('Ja, %s kommt mit zum Mittagessen' % getUserName, '7iqsqeqaefdmdcy4nj7ccnwfnw')
    elif kommtMit == "Vielleicht":
   	message.send('Vielleicht kommt %s mit zum Mittagessen' % getUserName, '7iqsqeqaefdmdcy4nj7ccnwfnw')
    else:
        message.send('%s Kommt nicht mit zum Mittagessen' % getUserName, '7iqsqeqaefdmdcy4nj7ccnwfnw')
